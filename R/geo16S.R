...
#  text(1, 1, "9. Calculate and record Zc and nH2O for all taxonomic groups at each level", adj = 0)
  ...
  metrics <- read.csv(system.file("extdata/RefSeq/taxon_metrics.csv.xz", package = "JMDplots"))
...
geo16S_S2 <- function(pdf = FALSE) {
  ...
  names <- read.csv(system.file("extdata/RefSeq/taxonomy.csv.xz", package = "JMDplots"))
  # Take out viruses
...
geo16S_S4 <- function(pdf = FALSE) {
  ...
  # Read NCBI names
  NCBI <- read.csv(system.file("extdata/RefSeq/taxonomy.csv.xz", package = "JMDplots"))
