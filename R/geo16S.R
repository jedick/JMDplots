...
geo16S2 <- function(pdf = FALSE) {
  ...
  legend("bottomright", legend, pch = c(24, 20, 21), col = c(1, 1, 1), pt.bg = c(3, NA, 4), bg = "white", cex = 0.8)
  ...
  p5 <- plotcomp("XDZ+17", title = FALSE, points = FALSE)
  ...
  title("Lake Fryxell microbial mats\nJungblut et al., 2016", font.main = 1)
  addhull(p6$ZC, p6$nH2O, 4, TRUE)
  pointfun(p6)
  legend <- c("oxic", "transition", "anoxic")
  legend("bottomright", legend, pch = c(24, 20, 25), pt.bg = c(4, 1, 2), bg = "white")

  p7 <- plotcomp("VAH+18", title = FALSE, points = FALSE)
  title("Laguna Potrok Aike sediment\nVuillemin et al., 2018", font.main = 1)
  addhull(p7$ZC, p7$nH2O, 4, TRUE, lty = 2)
  pointfun(p7)
  legend <- c("< 1 m", "2-71 m")
  legend("bottomright", legend, pch = c(24, 25), pt.bg = c(4, 2), bg = "white")

  p8 <- plotcomp("EH18", title = FALSE, points = FALSE)
  title("Mono Lake\nEdwardson and Hollibaugh, 2018", font.main = 1)
  addhull(p8$ZC, p8$nH2O, "tan1", TRUE)
  pointfun(p8)
  legend <- c("10 m", "15-25 m", "31 m")
  ...
  lmlines()
  addhull(p1$ZC, p1$nH2O, 2, TRUE, lwd = 2)
  addhull(p2$ZC, p2$nH2O, 1, TRUE, lwd = 2, lty = 2)
  addhull(p3$ZC, p3$nH2O, 1, TRUE, lwd = 2)
  addhull(p4$ZC, p4$nH2O, 2, TRUE, lwd = 2, lty = 2)
  addhull(p5$ZC, p5$nH2O, "tan1", TRUE, lwd = 2, lty = 2)
  addhull(p6$ZC, p6$nH2O, 4, TRUE, lwd = 2)
