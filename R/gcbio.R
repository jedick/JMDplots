...
gcbio5 <- function(pdf = FALSE) {
  ...
  nrow <- 7
  ncol <- 6
  elpos <- coordinates(rep(nrow, ncol), mx = -0.05, my = 0, hor = FALSE)
  # Size of the axis labels
  lcex <- 0.9

  del <- 0 * nrow
  textempty(elpos[1 + del, ], "Levels of\nOrganization", cex = lcex, font = 2, adj = 0)
  labels <- c("Fine\nScale", "", "", "", "", "Coarse\nScale")
  for(i in 1:6) textempty(elpos[i + 1 + del, ], labels[i], adj = 0)
  dx <- 0.02
  dy <- 0.08
  from <- elpos[2 + del, ] + c(dx, dy)
  to <- elpos[7 + del, ] + c(dx, -dy)
  straightarrow(from, to, arr.pos = 1, endhead = TRUE, lcol = "#366BFB", lwd = 3)
  straightarrow(to, from, arr.pos = 1, endhead = TRUE, lcol = "#366BFB", lwd = 3)

  del <- 1 * nrow
  textempty(elpos[1 + del, ], "Biological\nEntity", cex = lcex, font = 2, adj = 0)
  labels <- c("Molecule", "Macromolecule", "Cell/Organism", "Population", "Community", "Ecosystem")
  for(i in 1:6) textempty(elpos[i + 1 + del, ], labels[i], adj = 0)

  del <- 2 * nrow
  textempty(elpos[1 + del, ], "Process\nType", cex = lcex, font = 2, adj = 0)
  labels <- c("Chemistry", "Biochemistry", "Physiology", "Behavior", "Ecology", "Environment")
  for(i in 1:6) textempty(elpos[i + 1 + del, ], labels[i], adj = 0, col = "#366BFB")
  dx <- 0.11
  from <- elpos[2 + del, ] + c(dx, 0)
  to <- elpos[4 + del, ] + c(dx, 0)
  curvedarrow(from, to, curve = 0.1, arr.type = "none", lcol = "#366BFB", lwd = 3)
  from <- elpos[5 + del, ] + c(dx, 0)
  to <- elpos[7 + del, ] + c(dx, 0)
  curvedarrow(from, to, curve = 0.1, arr.type = "none", lcol = "#366BFB", lwd = 3)
  ...
  dy2 <- -0.05
  i <- 2; textempty(elpos[i + 1 + del, ] + c(0, dy2 + dy1), labels[i], adj = c(0, 1), col = "#D50000", font = 3)
  i <- 5; textempty(elpos[i + 1 + del, ] + c(0, dy2), labels[i], adj = c(0, 1), col = "#D50000", font = 3)

  del <- 5 * nrow
  textempty(elpos[1 + del, ], "Time\nScales", cex = lcex, font = 2, adj = 0)
  labels <- c("Cellular\nand Metabolic", "", "", "", "", "Evolutionary\nand Geological")
  for(i in 1:6) textempty(elpos[i + 1 + del, ], labels[i], adj = 0)
  dx <- 0.02
  dy <- 0.08
  from <- elpos[2 + del, ] + c(dx, dy)
  to <- elpos[7 + del, ] + c(dx, -dy)
  straightarrow(from, to, arr.pos = 1, endhead = TRUE, lcol = "#D50000", lwd = 3)
  straightarrow(to, from, arr.pos = 1, endhead = TRUE, lcol = "#D50000", lwd = 3)
