...
```{r utegigS3, out.width='80%', fig.align='center', fig.width = 6, fig.height = 8, pngquant = pngquant}
utegigS3()
```

## Convert `r logaH2` to Eh and `r logfH2`
```{r utegig_H2_Eh}
logaH2 <- -15
pH <- 7
logK1 <- suppressMessages(subcrt(c("H2", "H+", "e-"), c(-1, 2, 2), T = 25)$out$logK)
pe <- (- logK1 - 2 * pH - logaH2) / 2
Eh <- convert(pe, "Eh")
# For 10^-2 pH2
logK2 <- subcrt(c("H2", "hydrogen"), c(-1, 1), T = 25)$out$logK
logfH2 <- logK2 + logaH2
print(paste0("Given logaH2 = ", logaH2, " and pH = ", pH, ", Eh = ", round(1000 * Eh), " mV at equilibrium"))
print(paste0("Given logpH2 = ", logaH2, ", logfH2 = ", round(logfH2, 2), " at equilibrium"))
```

## Convert hydrogen partial pressure to `r logaH2`
```{r utegig_H2_H2}
logK <- subcrt(c("hydrogen", "H2"), c(-1, 1), T = 25)$out$logK
# For 10^-2 pH2
logpH2 <- -2
