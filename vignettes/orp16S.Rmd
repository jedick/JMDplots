...
**Post-publication update** on 2024-07-09: The reference database for constructing community reference proteomes has been changed from RefSeq 206 to GTDB 220.
Correspondingly, taxonomic classifications now use [RDP Classifier training files for 16S rRNA sequences from GTDB](https://zenodo.org/doi/10.5281/zenodo.7633099) instead of the default RDP Classifier training set. As a result of the slightly different values, the x-axis scale in Fig. 1c has been expanded, and the particular random selection of species has a slope further from zero in the center plot of this panel.
...
**NOTE**: With the switch to GTDB on both ends, this shows 100% mapping rate, higher than the values mentioned in the article for mapping between classifications using the RDP training set and NCBI taxonomy.
```{r mapperc}
