...
This vignette runs the code to make the plots from the following manuscript:

> Dick JM. 2024. Adaptations of microbial genomes to human body chemistry. Submitted manuscript.
> 
...
## Differences of `r nO2` and `r nH2O` between untreated and viral-inactivated samples (Figure S2)
...
## Chemical metrics of reference proteomes for genera with known oxygen tolerance (Figure S3)
...
## Differences of `r nO2` and `r nH2O` between subcommunities of obligate anaerobes and aerotolerant genera in controls and patients (Figure S4)
```{r microhum_S4, message = FALSE, out.width='100%', fig.align='center', fig.width = 10, fig.height = 3.5, pngquant = pngquant, dpi = 100}
microhum_S4()
```

## Summary of metagenome sequence processing (Table S2)
```{r microhum_table_S2}
